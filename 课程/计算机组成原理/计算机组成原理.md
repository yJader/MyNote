...
> 二编: cache地址结构中, **组内块号**这个部分应该不存在, 不知为何编辑错误, 待查证
>
> - CSAPP中同样没有, 且根据组相联访存流程 定位组之后, 用标签位一个个比较地址, 组内块号这个部分无用
...
2. 一个总线在某一时刻可以有多对主从设备进行通信吗？
   - 不可以。在某个总线周期内，总线上只有一个主设备控制总线，选择一个从设备与之进行通信（即一对一的关系），或对所有设备进行广播通信（即一对多的关系）。所以一个总线在某一时刻不能有多对主从设备进行通信，否则会发生数据 。

...
![f812aba316123e27823e7429d2b00433](6.824 2020 CourseNote.assets/f812aba316123e27823e7429d2b00433.png)
...
![img](6.824 2020 CourseNote.assets/GFS Architecture.png)
...
![append导致不一致状态](6.824 2020 CourseNote.assets/append导致不一致状态.png)
...
[![img](https://camo.githubusercontent.com/3ba0b86ec171153c9e3b782d9743d535e1cab5b930adcf9821e06c817dda2d86/68747470733a2f2f706963332e7a68696d672e636f6d2f38302f76322d34373363306339373832373961616464323063303136353434323661616438615f31343430772e77656270)](https://camo.githubusercontent.com/3ba0b86ec171153c9e3b782d9743d535e1cab5b930adcf9821e06c817dda2d86/68747470733a2f2f706963332e7a68696d672e636f6d2f38302f76322d34373363306339373832373961616464323063303136353434323661616438615f31343430772e77656270)

这里进行一个复杂场景的讨论，如果原本系统中有某个server宕机，剩余server中谁会成为leader？

下面用（X~~Z，Y）表示log index X~~Z的位置有term Y的记录
...
# Lec7 容错-Raft(Fault Tolerance-Raft) -2
...
> [7.2 选举约束（Election Restriction） - MIT6.824 (gitbook.io)](https://mit-public-courses-cn-translatio.gitbook.io/mit6-824/Lec-07-raft2/7.2-xuan-ju-yue-shu-election-restriction)
>
> 在Raft论文的5.4.1，Raft有一个稍微复杂的选举限制（Election Restriction）。这个限制要求，在处理其他节点(Candidate)发来的RequestVote RPC时，会与自己的日志进行**比较**，如果发现对方的日志**落后**于自己的日志，就会拒绝这次 RPC 调用
>
> - **比较方式**: 首先由最后的log的 Term ID 决定大小，在 Term ID 相同时由 index 决定大小
...
## 7.2 Log catch up(unoptimized)

- **nextIndex**：所有raft节点都维护nextIndex乐观的变量用于记录下一个需要填充log entry的log index。
  - 这里说乐观，因为当leader当选时，leader会初始化nextIndex值为当前log index值+1，表示认为leader自身的log一定是最新的

- **matchIndex**：leader为所有raft节点(包括leader自己)维护一个悲观的matchIndex用于记录leader和其他follower从0开始往后最长能匹配上的log index的位置+1，表示leader和某个follower在matchIndex之前的所有log entry都是对齐的。
  - 这里说悲观，因为leader当选时，leader会初始化matchIndex值为0，表示认为自身log中没有一条记录和其他follower能匹配上。随着leader和其他follower同步消息时，matchIndex会慢慢增加。
  - **leader为每个自己的follower维护matchIndex，因为平时根据majority规则，需要保证log已经同步到足够多的followers上**。


 假设这里有S1～S3三台服务器组成Raft集群，每个Server的log entries如下

| index             | 10   | 11   | 12   |
| ----------------- | ---- | ---- | ---- |
| S1                | 3    |      |      |
| S2(Term5的Leader) | 3    | 3    | 5    |
| S3                | 3    | 3    | 4    |
...
到这里为止，S2能够知道log index12的log entry term5至少在2个server上得到复制(S2和S3)，已经满足了majority原则了，所以S2能将消息传递到上层应用了。不幸的是，这不完全是对的。

**这里未优化的版本有个很大的问题，那就是如果Raft集群中出现log落后很多的server，leader需要进行很多次请求才能将其log与自己对齐**。(因为每次nextIndex--)



## 7.3 Log catch up quikly
...
**一种优化思路**: 在拒绝AppendEntries的reply中添加Follower自身的log信息, 以便下一次Append正确修改

| reply参数 | 描述                                                                                                                             |
| ------- | ------------------------------------------------------------------------------------------------------------------------------ |
| XTerm   | 对于RPC的args中prevLogIndex, 如果Follower在这个index的LogX的Term与prevLogTerm不匹配, 将XTerm等于LogX的Term<br />- 如果在prevLogIndex处没有log, XTerm=-1 |
| XIndex  | 在Follower中, 对于XTerm的第一条Log Entry的Index                                                                                         |
| XLen    | 如果prevLogIndex处没有log, 从此处开始向前数, 返回空Log槽位数                                                                                      |

**Case 1**: Leader有 XTerm：nextIndex = Leader对于 XTerm 的最后一个条目
- S1没有Term 6的任何Log，因此我们需要**回退**一整个Term的Log
...
- S2->S1: prevLogIndex = 3, prevLogTerm = 6
- S1->S2: XTerm = 5, **XIndex = 2**
- S2: 自己没有Term5的Log, 设置nextIndex = XIndex = 2

**Case 2**: Leader没有 XTerm：nextIndex = XIndex
- S1收到了任期4的旧Leader的多条Log，但是作为新Leader，S2只收到了一条任期4的Log。所以这里，我们需要覆盖S1中有关旧Leader的一些Log。
...
- S2->S1: prevLogIndex = 3, prevLogTerm = 6
- S1->S2: XTerm = 4, **XIndex = 1**
- S2: 自己有XTerm(即4)的Log, nextIndex=最后一条XTerm(即4)Log的下一条(即2)
...
- S2->S1: prevLogIndex = 3, prevLogTerm = 6
- S1->S2: XTerm = -1, **XLen = 2**
- S2: S1日志有空白, 不能判断空白前有没有冲突, 先发空白后的所有Log试试, nextIndex = nextIndex - XLen = 2



## 7.4 持久化(Persistence)

> [7.4 持久化（Persistence） - MIT6.824 (gitbook.io)](https://mit-public-courses-cn-translatio.gitbook.io/mit6-824/Lec-07-raft2/7.4-chi-jiu-hua-persistent)
>
...
2024版增加了一节单独的一致性课程: [Lec ?: Consistency and Linearizability](# Lec ?: Consistency and Linearizability)
...
1. **整体操作顺序一致(total order of operations)**
   ...
   即使操作实际上并发进行，你仍然可以按照整体顺序对它们进行排序。（即**后续**可以根据读写操作的返回值，对所有读写操作**整理**出一个符合逻辑的整体执行顺序）
...
1. API -> 通用的协调服务(Coordination service)
2. 高性能(high perference) ：
	- 异步(asynchronous)
	- 提供一致性，但非强一致性(consistency)

N个机器能提高N倍的性能吗?
- 显然不能, Leader节点存在瓶颈, 多服务器会带来更多的开销, 性能反而下降
一个简单的优化: 只有write走Leader, read走副本(其他服务器)
随之而来的另一个问题: 没有办法保证Leader之外的任何一个副本的数据是最新(up to data)的

Zookeeper的选择: **放弃线性一致性**, 不保证读请求提供最新的数据


## 8.1 Zookeeper复制状态机(replicated state machine)
...
2. Zookeeper调用**ZAB库**(类似Raft库)，生成操作log，ZAB负责和Zookeeper集群的其他机器完成类似Raft的工作，日志同步、心跳保活等(整体思想和Raft类似，但实现方式不同)
   - ZAB保证日志顺序
   - 能避免网络分区、脑裂问题
...
> [8.5 一致保证（Consistency Guarantees） - MIT6.824 (gitbook.io)](https://mit-public-courses-cn-translatio.gitbook.io/mit6-824/Lec-08-zookeeper/8.5) <= 网络博主的笔记，这里摘抄部分，补充说明
>
> Zookeeper的一致性规则：
>
...
对于**异步**写请求, 会按照client指定的顺序执行(类似于打上序号)

对于读请求, 假设有多个操作ReadA, ReadB, ReadC... C访问任何一个副本时, 都会Read到比A, B相同或更新的状态
- 通过携带一个zxid对副本请求, 如果副本的Log低于这个zxid, 会等待自身Log更新到这个zxid后再进行返回(也可能直接拒绝)
...
> [8.6 同步操作（sync） - MIT6.824 (gitbook.io)](https://mit-public-courses-cn-translatio.gitbook.io/mit6-824/Lec-08-zookeeper/8.6-tong-bu-cao-zuo-sync)
>
...
> 不管怎么样，如果我需要读最新的数据，我需要发送一个sync请求，之后再发送读请求。这个读请求可以保证看到sync对应的状态，所以可以合理的认为是最新的。但是同时也要认识到，这是一个代价很高的操作，因为我们现在将一个廉价的读操作转换成了一个耗费Leader时间的sync操作。所以，如果不是必须的，那还是不要这么做。
 ...
 论文中没有特别详细的说明Zookeeper怎么实现"8.3 Zookeeper的一致性规则"中制定的规则，但是我们可以大致猜测其实现。
...
>
> **需要注意的是客户端的 Watch 机制是一次性的，触发后就会被删除。**
>
> [zookeeper的watch机制原理解析_java_脚本之家 (jb51.net)](https://www.jb51.net/article/252975.htm) <= 推荐阅读，具体的指令操作示例，很好理解
>
> ------
>
> [8.7 就绪文件（Ready file/znode） - MIT6.824 (gitbook.io)](https://mit-public-courses-cn-translatio.gitbook.io/mit6-824/Lec-08-zookeeper/8.7-jiu-xu-wen-jian-ready-fileznode) 
> 如何保证read读到的一定是最新的write -- 以Ready file为例

 Ready file类似一把锁, 当存在时, 其他client可以去读配置文件f, 当不存在时, 代表有人正在更新配置文件f, 需要等待Ready file创建完成
-  一个理想的修改流程: 如上面描述![](6.824-2020-CourseNote.assets/IMG-6.824-2020-CourseNote-20241216170704403.png)
- 一个很可能发生的错误的流程: read客户端没有观察到ready file被删除了![](6.824-2020-CourseNote.assets/IMG-6.824-2020-CourseNote-20241216172003978.png)
这种情况下, zookeeper提供了watch机制
- ClientA向某个副本watch了某个Ready file(`exists(ready, watch=true)`)，之后ClientA又发送了一些读请求(`read f1; read f2`)
- 当这个副本执行了一些会触发watch通知的请求(`delete ready`)，那么Zookeeper可以保证
	1. 这个副本会先发给ClientA watch对应的通知
	2. 再处理触发watch通知的请求（`delete ready`）
	3. 然后再处理ClientA的后续请求
	4. ![](6.824-2020-CourseNote.assets/IMG-6.824-2020-CourseNote-20241216174818520.png)
 
watch创建的流程
- exist请求在交互的副本上创建一个watch表单, 表明哪些客户端watch了哪些znode
- 如果收到一个删除Ready file的请求, 该副本会查看watch表单, 查找是否有file对应的watch
- 注: watch是基于一个特定的zxid建立的


## 8.6 Zookeeper API

### Zookeeper的应用
> [9.1 Zookeeper API - MIT6.824 (gitbook.io)](https://mit-public-courses-cn-translatio.gitbook.io/mit6-824/Lec-09-more-replication-craq/9.1-zookeeper-api) <= 可以参考这个笔记，里面更详细
>
> 回忆一下Zookeeper的特点：
...
Zookeeper的API Znode看起像文件系统, 有一个层次化的目录结构, 具有根目录, 可以在下面创建APP子目录和其他目录, 并包含文件
但是
...
- create接口，参数为path、data、flags。`create(path, data, flags)`，这里flags对应上面ZNode的3种类型
	- 创建是排他的, 只有成功创建才会返回yes, 已存在则返回no
- delete接口，参数为path、version。`delete(path, version)`
	- 每个znode都带有版本号version, 如果传入version参数, 则表明只有当znode的version=传入的version相同时, 才进行操作
- exists接口，参数为path、watch。`exists(path, watch)`
	- 无论文件是否存在, 都会创建一个watch, 以监听指定文件的创建, 删除, 修改操作
...
### 用Zookeeper实现计数器 (mini transaction)
...
while True:
    ...
    if exist("f", watch=TRUE):
        WAIT
```
同样有羊群效应, 多个client可能会同时进行create操作
### 可扩展锁 Scalable Lock
...
def Lock():
	# 自动递增, 创建出如f15的文件
	fn = create("f", data, sequential=TRUE, ephemeral=TRUE) 
	while True:
	    files = LIST("f*") # 列出所有f开头的文件
	    if NO LOWER THAN fn IN files: # 当前的fn就是最小的, 说明获取到锁了
		    return 
	    if EXIST(NEXT LOWER fn, watch=TRUE): 
	        WAIT
...
实现了顺序等待, fn只有当fn-1释放锁时才会获取锁(return Lock函数)
- 如果fn-1文件存在, 就说明①fn-1在等待前面的释放锁②fn-1占有锁
- 实际上可能不是n-1 因为如果创建fn-1的client断连, 会自动删除关联的临时文件, 此时等待的就是fn-2

---
> 教授提出: Zookeeper论文中用锁的例子不太恰当, 因为这个锁的使用者-Client的性质导致其不能保证原子性: Client在上锁期间可能执行了一些写操作, 然后因为各种意外与Zookeeper断开连接(如果没有在意外退出前清理数据), 但是线程会正确的释放锁
> - 如果出现线程再释放锁之前crash, 那么说明当前的程序故障, 想要恢复是很困难的, 最好直接kill

这种锁的应用场景: Soft Lock
- 举个例子，当你在运行MapReduce Job时，你可以用这样的锁来确保一个Task同时只被一个Work节点执行。例如，对于Task 37，执行它的Worker需要先获得相应的锁，再执行Task，并将Task标记成执行完成，之后释放锁。MapReduce本身可以容忍Worker节点崩溃，所以如果一个Worker节点获得了锁，然后执行了一半崩溃了，之后锁会被释放，下一个获得锁的Worker会发现任务并没有完成，并重新执行任务。
...
# Lec9 链式复制(Chain Replication)和CRAQ

## 9.0 两种构造RSM(复制状态机)的方案
...
1. 读操作/查询操作(read operation / query operation)只涉及一个服务器(reads ops involve 1 server)
	- 而Raft的写需要Leader向所有Follower去发送Log
2. 具有简单的恢复方案(simple recovery plan)
3. 线性一致性(linearizability)

 整体来说，这是一个很成熟的设计，已经应用到诸多系统中。

[![img](https://camo.githubusercontent.com/9974171258899a8e39c1b7e4c5d4118fa40198f6eb38d8f09e2e2a8b4725d45d/68747470733a2f2f706963312e7a68696d672e636f6d2f38302f76322d34663032396339386536643936336162363233333535376463383366626132635f31343430772e77656270)](https://camo.githubusercontent.com/9974171258899a8e39c1b7e4c5d4118fa40198f6eb38d8f09e2e2a8b4725d45d/68747470733a2f2f706963312e7a68696d672e636f6d2f38302f76322d34663032396339386536643936336162363233333535376463383366626132635f31343430772e77656270)

 在链式复制系统中，首先有一个**配置服务**(configuration service)记录链式连接的节点信息。
 - 配置服务的权限类似于master, 由Raft/Paxos实现, 可以解决链式复制中的网络分区问题
...
- **写请求总是发送到head节点**
  ...
  head修改storage中的state后，向下传递操作，直到tail节点。中间的节点总是更新state，然后向下传递操作。最后**由tail节点响应client**。
...
1. 整个系统以简单的链表维护
2. 配置相关的工作交给了**专门的配置服务**
	- 由配置服务来决定S123的状态改变, 防止分区导致的错误(如脑裂)

## 9.5 链式复制-新增副本(add replica)
...
4. 简单的崩溃恢复机制(simple crash recovery):  
	- 与Raft相比，CR的崩溃处理更简单。
...
## 9.7 链式复制-并行读优化拓展(Extension for read parallelism)
...
> [9.7 链复制的配置管理器（Configuration Manager） - MIT6.824 (gitbook.io)](https://mit-public-courses-cn-translatio.gitbook.io/mit6-824/Lec-09-more-replication-craq/9.7-lian-fu-zhi-de-pei-zhi-guan-li-qi-configuration-manager) <= 摘抄我觉得有意义的问答内容：
>
...
**问题：我想知道前面"9.5 链式复制-新增副本(add replica)"增加tail时，S3什么时候能成为新的tail，有没有可能S2一直是tail，S3一直没同步完原tail(S2)的数据？**
...
# Lec10? Patterns and Hints for Concurrency in Go
...
# Lec12 缓存一致性(Cache Consistency)-Frangipani
...
> [11.4 缓存一致性（Cache Coherence） - MIT6.824 (gitbook.io)](https://mit-public-courses-cn-translatio.gitbook.io/mit6-824/Lec-11-cache-consistency-frangipani/1.4-huan-cun-yi-zhi-xie-yi-coherence-protocol) <= 摘抄一些内容，作为补充说明
>
...
> [11.5 原子性（Atomicity） - MIT6.824 (gitbook.io)](https://mit-public-courses-cn-translatio.gitbook.io/mit6-824/Lec-11-cache-consistency-frangipani/11.5-yuan-zi-xing-atomicity) <= 摘抄部分内容，作为补充说明
>
...
> [11.6 Frangipani Log - MIT6.824 (gitbook.io)](https://mit-public-courses-cn-translatio.gitbook.io/mit6-824/Lec-11-cache-consistency-frangipani/11.6-gu-zhang-hui-fu-crash-recovery) <= 摘抄部分内容，作为补充说明
>
...
> [11.7 故障恢复（Crash Recovery） - MIT6.824 (gitbook.io)](https://mit-public-courses-cn-translatio.gitbook.io/mit6-824/Lec-11-cache-consistency-frangipani/11.7-gu-zhang-hui-fu-crash-recovery) <= 摘抄部分内容，作为补充说明
>
...
# Lec13 分布式事务(Distributed Transaction)
...
> [12.1 分布式事务初探（Distributed Transaction） - MIT6.824 (gitbook.io)](https://mit-public-courses-cn-translatio.gitbook.io/mit6-824/Lec-12-distributed-transaction/12.1) <= 摘抄部分内容，作为补充说明
>
...
- Wide-area transation：支持广域的事务，即跨越多个国家/地区的分布式事务
  ...
  - Read-only transaction：只读事务可以在任意数据中心运行，大约比读写事务快10倍（Spanner针对只读事务做优化）
 ...
 *补充说明：线性一致性要求如果操作2在操作1结束后才开始，那么操作2必须排在操作1之后。详情见“7.7 线性一致性/强一致性(Linearizability/strong consistency)”*
 ...
 对比**事务的外部一致性(external consistency)和线性一致性(linearizability)**：
...
# Lec15 乐观并发控制-FaRM(Optimistic Concurrency Control)
...
[![FaRM论文笔记](https://camo.githubusercontent.com/f8a044771b4bd4df7fd3b3c9c6a314296339fa6fb8e348cfad2a1033559e55c1/68747470733a2f2f706963782e7a68696d672e636f6d2f76322d39396363316561353232386661323865323833393666636433616439623739325f373230772e6a70673f736f757263653d3137326165313862)](https://camo.githubusercontent.com/f8a044771b4bd4df7fd3b3c9c6a314296339fa6fb8e348cfad2a1033559e55c1/68747470733a2f2f706963782e7a68696d672e636f6d2f76322d39396363316561353232386661323865323833393666636433616439623739325f373230772e6a70673f736f757263653d3137326165313862)

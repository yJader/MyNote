...
定义伴随值 $\bar{v}_i = \frac{\partial y}{\partial v_i}$，然后按照计算图的反向拓扑顺序迭代计算 $\bar{v}_i$：
...
定义部分伴随值 $\bar{v}_{i \to j} = \bar{v}_j \frac{\partial v_j}{\partial v_i}$ 用于每对输入-输出节点，则：

$$\bar{v}_i = \sum_{j \in next(i)} \bar{v}_{i \to j}$$

我们可以分别计算部分伴随值，然后将它们相加。
...
def gradient(out):
    ...
    for i in reverse_topo_order(out):
        v̄_i = sum(node_to_grad[i])
        for k in inputs(i):
            compute v̄_k→i = v̄_i * ∂v_i/∂v_k
            append v̄_k→i to node_to_grad[k]
    return adjoint of input v_input
```

## 通过扩展计算图实现反向模式 AD

反向模式 AD 可以通过构建一个计算图来计算伴随值实现。这个过程中，我们扩展原始计算图，添加用于计算伴随值的节点。
...
对于张量值，定义伴随张量 $\bar{Z} = \begin{bmatrix} \frac{\partial y}{\partial Z_{1,1}} & \cdots & \frac{\partial y}{\partial Z_{1,n}} \ \vdots & \ddots & \vdots \ \frac{\partial y}{\partial Z_{m,1}} & \cdots & \frac{\partial y}{\partial Z_{m,n}} \end{bmatrix}$
...
对于数据结构，我们也可以定义相应的伴随数据结构。例如，对于字典：

- 前向计算：$d = {"cat": a_0, "dog": a_1}$，$b = d["cat"]$，$v = f(b)$
- 反向评估：$\bar{b} = \frac{\partial v}{\partial b} \bar{v}$，$\bar{d} = {"cat": \bar{b}}$

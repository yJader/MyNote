...
- L层通用结构：
  $$
  \begin{aligned}
  Z_{i+1} &= \sigma_{i}\left(Z_{i} W_{i}\right),\quad i=1,\ldots,L \\
  Z_{1} &= X \\
  h_{\theta}(X) &= Z_{L+1}
  \end{aligned}
  $$
  - 维度关系：$Z_i\in\mathbb{R}^{m\times n_i},\ W_i\in\mathbb{R}^{n_i\times n_{i+1}}$
...
### 梯度计算框架
- 优化目标：
  $$\min_{\theta} \frac{1}{m}\sum_{i=1}^{m}\ell_{ce}\left(h_{\theta}(x^{(i)}), y^{(i)}\right)$$
  - 核心需求：计算$\nabla_\theta\ell_{ce}$
...
#### 对$W_1$的梯度
$$
\nabla_{W_1}\ell = X^T\left[ (S-I_y)W_2^T \circ \sigma'(XW_1) \right]
$$
- $\circ$表示逐元素乘法
- $\sigma'$为激活函数导数
...
\begin{aligned}
Z_1 &= X \\
Z_{i+1} &= \sigma_i(Z_iW_i)\quad (i=1,...,L)
\end{aligned}
$$
...
### 计算图视角
- ​**向量-雅可比积**：核心计算单元
  $$\frac{\partial Z_{i+1}}{\partial Z_i} = \sigma_i'(Z_iW_i) \cdot W_i$$

...
\end{aligned}
$$
- **偏差修正**（unbiased）：$$ \theta_{t+1} = \theta_t - \alpha \frac{u_{t+1}}{1-\beta^{t+1}} $$
	- 在训练的初期, 历史梯度不够大, 导致动量不够有效, 加上修正能够提高收敛速度

  ![image-20250817181507556](./10-414 深度学习系统课程笔记.assets/image-20250817181507556.png)
### Nesterov 动量
- 在「未来点」计算梯度：
...
### 随机初始化的影响
- 以 $W_i \sim \mathcal{N}(0,\sigma^2 I)$ 为例：
  - **方差 $\sigma^2$ 决定**：
    1. 前向激活的范数 $\|Z_i\|$；
    2. 反向梯度的范数 $\|\nabla_{W_i}\ell\|$。
...
- 假设 $x\sim\mathcal{N}(0,1)$，$w\sim\mathcal{N}(0,\frac{1}{n})$，则：
  - $\mathbb{E}[w^T x]=0,\quad \mathrm{Var}(w^T x)=1$。

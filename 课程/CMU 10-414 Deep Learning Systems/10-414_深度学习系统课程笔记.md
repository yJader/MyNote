...
**计算过程示例**:
1.  计算输出 $y_{11}$:
    ![此处应插入 PPT 第 8 页关于 $y_{11}$ 计算的图示](10-414_深度学习系统课程笔记.assets/IMG-10-414_深度学习系统课程笔记-20250825163728578.png)
...
[此处应插入 PPT 第 19 页的图示，展示 Max pooling 和 Strided convolution 如何降低图像分辨率]
...
[此处应插入 PPT 第 20 页的图示，展示分组卷积如何将输入和输出通道分组进行独立的卷积操作]
...
[此处应插入 PPT 第 21 页的图示，展示一个常规卷积和一个扩张卷积的对比，后者在相同的参数下有更大的感受野]
...
* 换句话说，对于矩阵向量乘法操作 `Wx`，计算反向传播 (`backwards pass`) 需要乘以转置矩阵 $W^T$

...
$L_n(x)=\sum_{k=0\\}^ny_kl_k(x): Lagrange插值多项式\\
l_k(x)=\frac{\omega_{n+1}(x)}{\omega_{n+1}^{\prime}(x_k)(x-x_k)}: Lagrange插值基函数$ 
...
- N 个条件可以确定N-1阶多项式
- 要求在1个节点 x0 处直到**m 阶导数**都重合的插值多项式即为Taylor多项式
  - $\begin{gathered}
...
因为$H_3(x)$是3次多项式, 所以$\alpha_0(x)$最高也为3次, $\begin{cases}\alpha_0(x_0)=1&\alpha_0'(x_0)=0\\\alpha_0(x_1)=0&\alpha_0'(x_1)=0\end{cases}$

- 在$x_1$处, $\alpha_0(x_1)=\alpha_0'(x_1)=0$, 即$x_1$为$\alpha_0$的**二重零点**, 函数值为0, 导数也为0
  可设$\color{blue}\alpha_0(x)=(x-x_1)^2(\alpha x+b)$ 
- 在$x_0$处, $\alpha_0(x_0)=1\ \ \alpha_0'(x_0)=0$ , 代入得$a=-\frac2{\left(x_0-x_1\right)^3}\quad b=\frac1{\left(x_0-x_1\right)^2}+\frac{2x_0}{\left(x_0-x_1\right)^3}$

...
首先MySQL是典型的C/S架构，即<font color='#66ccff'>Clinet/Server 架构</font>，服务端程序使用的mysqld。
...
![image-20220615133227202](IMG-20241208161954858.png)

下面具体展开如下：

![image-20220615133420251](IMG-20241208161955293.png)
...
![image-20220615140556893](IMG-20241208161955665.png)
...
![image-20220615140710351](IMG-20241208161955948.png)
...
<img src="MySQL架构篇.assets/image-20220615141934531.png" alt="image-20220615141934531" style="zoom: 67%;" />

#### ① 查询缓存

1. <font color='#66ccff'>查询缓存</font>: Server 如果在查询缓存中发现了这条 SQL 语句，就会直接将结果返回给客户端；如果没有，就进入到解析器阶段。

2. 需要说明的是，因为查询缓存往往效率不高，<font color='#EE0000'>所以在 MySQL8.0 之后就抛弃了这个功能</font>。

   > **总之，因为查询缓存往往弊大于利，查询缓存的失效非常频繁。**

3. 一般建议大家在<font color='#66ccff'>静态表</font><font color='orange'>(一般我们极少更新的表)</font> 里使用查询缓存。比如，一个系统配置表、字典表，这张表上的查询才适合使用查询缓存。

   * 好在MySQL也提供了这种“按需使用”的方式。你可以将 `my.cnf` 参数 `query_cache_type` 设置成 `DEMAND`，代表当 sql 语句中有 SQL_CACHE关键字时才缓存。比如：
   ...
   * 而对于你确定要使用查询缓存的语句，可以供SQL_CACHE显示指定，像下面这个语句一样：
   ...
   * 监控查询缓存的命中率：
        ...
        ```
	     ...
	     <img src="MySQL架构篇.assets/image-20220615144537260.png" alt="image-20220615144537260" style="zoom: 67%;" />
...
![image-20220615142301226](IMG-20241208161956223.png)
...
![image-20220615162031427](IMG-20241208161956570.png)

下图是SQL分词分析的过程步骤:

![image-20220615163338495](IMG-20241208161956915.png)
...
![image-20220615162613806](IMG-20241208161957229.png)
...
![image-20220615164722975](IMG-20241208161957490.png)
...
![image-20220615172149919](IMG-20241208161957751.png)
...
![image-20220615172409967](IMG-20241208161958070.png)
...
![image-20220615172438338](IMG-20241208161958379.png)
...
![image-20220615173727345](IMG-20241208161958661.png)
...
![image-20220615173803835](IMG-20241208161958864.png)

```mysql
mysql> show profile for query 2;
```

![image-20220615173822079](IMG-20241208161959062.png)
...
![image-20220615175309751](IMG-20241208161959266.png)
...
![image-20220615193131719](IMG-20241208161959500.png)
...
![image-20220615222455867](IMG-20241208161959682.png)
...
![image-20220615223831995](IMG-20241208161959882.png)
...
![image-20220616125342599](IMG-20241208162000122.png)
...
![image-20220616125448555](IMG-20241208162000318.png)
...
![image-20220616125928861](IMG-20241208162000518.png)

![image-20220616125945304](IMG-20241208162000711.png)

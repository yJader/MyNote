...
- 不能相信前端发送的内容
- devtools会和redis有[冲突](http://t.csdn.cn/esklt) 
  - 改用JRebel `Ctrl+Shift+F9`实现热部署
...
2. 创建application.yml配置文件
   ...
   spring:
     datasource:
       url: jdbc:mysql://localhost:3306/yj_blog
...
3. 创建实体类，Mapper，Service 

   1. `/domain/entity/Article.java`

      ```java
      @Data
      @NoArgsConstructor
      @AllArgsConstructor
      @TableName("yj_article")
...
​	我们可以暂时先注释掉`framework`中的`SpringSecurity配置`依赖方便测试
...
![image-20230209211709250](media/image-20230209211709250.png)
...
2. 代码实现
    ...
    - 快捷键
      ...
      ![image-20230209220924280](media/image-20230209220924280.png)



3. 解决跨域问题

   ![image-20230209233225546](media/image-20230209233225546.png)
...
- 自己重新写一个sql也可以 但是麻烦 <font color='orange'>且并没有什么性能优化(大概)</font>
...
![image-20230210160012490](media/image-20230210160012490.png)

#### 3.2.1 需求

![image-20220202111056036](media/image-20220202111056036-16437714601701.png)

- 页面上需要展示分类列表，用户可以点击具体的分类查看该分类下的文章列表。

- 注意： 
  1. 要求只展示有发布正式文章的分类(不为草稿) 
  2. 必须是正常状态的分类
...
- 有正式文章的分类需要查询article表中的status字段 多表联查
- 应该尽量避免多表联查
...
- 添加`@Accessors(chain = true)` 使其适配map()

  <img src="media/image-20230210222157326.png" alt="image-20230210222157326" style="zoom:50%;" /> 



#### 3.3.4 FastJson配置

- 时间格式有问题 需要对Date->Json这个流程进行配置
  - 使用默认转换器进行时间转换 : `"createTime": "2022-01-23T15:20:11.000+00:00"`
...
public HttpMessageConverter fastJsonHttpMessageConverters() {
    ...
    FastJsonConfig fastJsonConfig = new Config();
...
- 注意配置`passwordEncoder`为`BCryptPasswordEncoder` (加密方法)
...
`WebSecurityConfigurerAdapter`已废弃 改用

1. 改用`AuthenticationConfiguration`的`authenticationConfiguration.getAuthenticationManager()`来暴露AuthenticationManger
2. 改用`filterChain()`进行配置并注册为Bean 而非使用父类的filter方法
3. [干货文：Spring Security 5.7+ 最新配置方案 - 掘金 (juejin.cn)](https://juejin.cn/post/7105992613559762981)
...
1. 认证失败处理器
   ...
   - 访问时未携带token(即权限不足 / 未登录) : `InsufficientAuthenticationException异常`
   - 用户名/密码错误 : `BadCredentialsException` 
   - 详情可以见异常源码
...
​	实际我们在开发过程中可能需要做很多的判断校验，如果出现了非法情况我们是期望响应对应的提示的。但是如果我们每次都自己手动去处理就会非常麻烦。我们可以选择直接抛出异常的方式，然后对==异常进行统一处理==。统一将异常中的信息封装成`ResponseResult`响应给前端。

​	

1. 自定义异常类 : SystemException
...
- 我的修改
  ...
  也可以在Config中添加`.antMatchers("/logout").authenticated()`以实现同样功能
...
![image-20220208214106296](media/image-20220208214106296.png)
...
​	建表SQL及初始化数据见：SGBlog\资源\SQL\sg_comment.sql	![image-20230215002702366](media/image-20230215002702366.png)
...
- 与普通评论共用相同的Service 在请求参数中添加区分评论类型的`commentType`参数
...
**不需要参数, 从token中解析获取userid 防止被伪造参数访问** 
...
![image-20220227224537701](media/image-20220227224537701.png)

![image-20220227224443813](media/image-20220227224443813.png)



秘钥

![image-20220228230512598](media/image-20220228230512598.png)

![image-20220228230933808](media/image-20220228230933808.png)
...
- 这里改用了`@PropertySource("classpath:key.properties")`来指定配置文件位置 便于将其ignore, 防止push泄露密钥

  - 注意 `@PropertySource("classpath:key.properties")`默认支持xml和properties 不支持yml

    原因是`@PropertySource()`默认使用`DefaultPropertySourceFactory`此`PropertySourceFactory`不支持yml

    若需要实现 则在注解中指定factory `@PropertySource(value = "classpath:foo.yml", factory = YamlPropertySourceFactory.class)`
...
~~~~java
    ...
    public ResponseResult register(User user) {
        ...
        if (isExist(user.getEmail(), User::getEmail)) {
            throw new SystemException(AppHttpCodeEnum.EMAIL_EXIST);F
        }
        //密码加密
...
![image-20220313133714102](media/image-20220313133714102.png)
...
![image-20230216203715973](media/image-20230216203715973.png)



![image-20230216203649086](media/image-20230216203649086.png)
...
- mysql读锁(共享锁)&写锁(排他锁) : 读锁多个事务共享一把锁, 写锁当一个事务获取了一个数据行的排他锁时, 其他事务就不能再获取该行的其他锁
- 直接操作时使用写锁, 高并发下会对性能造成影响

如何去优化呢？

1. 在==应用启动==时把博客的浏览量存储到redis中
...
1. 使用`@EnableScheduling`注解开启定时任务功能
...
2.  确定定时任务执行代码，并配置任务执行时间
    ...
    使用@Scheduled注解标识需要定时执行的代码。注解的cron属性相当于是任务的执行时间。

    目前可以使用 `0/5 * * * * ?` 进行测试，代表从0秒开始，每隔5秒执行一次。 
...
很多情况下我们可以用 ：[Cron - 在线Cron表达式生成器](http://cron.ciding.cc/)来帮助我们理解cron表达式和书写cron表达式。
...
1. **通用特殊字符：, - * /**  (可以在任意部分使用)
   ...
   2. `,`可以用来定义**列表**
      ...
      例如 ：  `1,2,3 * * * * ?` 表示 “ 每年每月每天每时每分的每个第1秒，第2秒，第3秒 ” 。

   3. `-`定义**范围**

      例如：`1-3 * * * * ?` 表示 “ 每年每月每天每时每分的第1秒至第3秒 ”。

   4. `/`**每隔多少**

      例如`5/10 * * * * ?` 表示 “ 每年每月每天每时每分，从第5秒开始，每10秒一次 ” 。
      即 “ / ” 的左侧是开始值，右侧是间隔。如果是从 “ 0 ” 开始的话，也可以简写成 “ /10 ” 
...
2. 日期部分还可允许特殊字符： `? ` `L ` `W`
   ...
   1. `?`: 表示没有具体的值
      ...
      - 使用?要注意**冲突**。日期和星期两个部分如果其中一个部分设置了值，则另一个必须设置为 “ ? ”。
        例如：`0\* * * 2 * ?`和 `0\* * * ? * 2` 
   ...
   3. `L`: 表示最后（Last）,只能用在日期和星期中
      - 在日期中表示每月最后一天，在一月份中表示31号，在六月份中表示30号
      - 也可以表示每月倒是第N天。
        例如： L-2表示每个月的倒数第2天
      - LW可以连起来用。 
        如`0 0 0 LW * ?` 表示每月最后一个工作日，即每月最后一个星期五
      - L在星期中表示7即星期六
        如`0 0 0 ? * L` 表示每个星期六
        `0 0 0 ? * 6L`若前面有其他值的话，则表示最后一个星期几，即每月的最后一个星期五

   4. `#` : 只能用在星期中，表示第几个星期几
      - 如`0 0 0 ? * 6#3` 表示每个月的第三个星期五。
...
![image-20230216230719499](media/image-20230216230719499.png)
...
//剩下的articleList和hotArticleList方法也使用getViewCount()修改了 但是太长 就不放出来了
~~~~

- 注意 redis中没有long类型 存取后会变成Integer

- 文章详情 : <img src="media/image-20230217002622678.png" alt="image-20230217002622678" style="zoom:33%;" /> 

  热门文章 : <img src="media/image-20230217002654323.png" alt="image-20230217002654323" style="zoom:33%;" /> 

  分类文章 : <img src="media/image-20230217002710717.png" alt="image-20230217002710717" style="zoom:33%;" /> 

  mysql : ![image-20230217002740784](media/image-20230217002740784.png)

  redis : <img src="media/image-20230217002810072.png" alt="image-20230217002810072" style="zoom:50%;" />
...
- SpringBoot2.6+ 使用`PathPatternMatcher`匹配路径，Swagger引用的`Springfox`基于`AntPathMatcher`匹配路径。匹配方式不同，导致错误。

- 解决方案 : 

- 将SpringBoot的匹配路径方式更改为`AntPathMatcher`，两者相同即可。添加配置信息如下：
...
- dto: Data Transfer Object, 数据传输(前后端之间的传输)对象
...
yj1 (超级管理员) `eyJhbGciOiJIUzI1NiJ9.eyJqdGkiOiI3NjZkMjljOGEzMTA0NTNlOGYxMjMzZjk2NjA2MmYzZCIsInN1YiI6IjEiLCJpc3MiOiJ5aiIsImlhdCI6MTY3ODE5MzUxMywiZXhwIjoxNjc4Mjc5OTEzfQ.72c4xTv2-YIs4KUKG73KrMs2cIIfJdm-6ynNKBoe0uE`

yj2 (普通用户)

`eyJhbGciOiJIUzI1NiJ9.eyJqdGkiOiIzYzRmOWU1OTU4YzM0NmFkYmU1MTNjMzQ3MzJiNWM1NyIsInN1YiI6IjIiLCJpc3MiOiJ5aiIsImlhdCI6MTY3ODIwMTM3MiwiZXhwIjoxNjc4Mjg3NzcyfQ.DPTtAJU9nRbJFciyveGjQxiC_sLKyvlYv2RBcTc_HNI`
...
PermissionService - hasPermisson()
...
- 奇怪的点: 改了Vo之后必须重启, JRebel热部署不能更新Vo
...
- 备注 : 前端工程有bug, 角色勾选后需要修改其他字段才会更新样式

...
[$Y = X @ W$, 其中 X 被按列切分, W 被按行切分]

- 这是一个 $Y = X_1@W_1 + X_2@W_2$ 的情况
...
- $Y$ (输出) 是 replicated (复制的)
  

### TP 解决方案 3: 组合模式 (Transformer FFN)

[$X \text{ (repl)} \rightarrow W1 \text{ (row-sharded)} \rightarrow Y \text{ (row-sharded)} \rightarrow W2 \text{ (col-sharded)} \rightarrow Z_{\text{partial}} \rightarrow \text{allreduce} \rightarrow Z \text{ (repl)}$]

...
[13_hardware_acceleration_architecture_overview.ipynb - Colab](https://colab.research.google.com/github/dlsyscourse/lecture13/blob/main/13_hardware_acceleration_architecture_overview.ipynb) 
...
## 15.1 序列建模 (Sequence Modeling)

### 序列建模任务

- 到目前为止我们考虑的例子中，我们都假设每个输入输出对 $x^{(i)}$, $y^{(i)}$ 是**独立同分布** (independent identically distributed, i.i.d.) 的
- 在实践中，很多情况下输入/输出对是按特定序列给出的，我们需要利用这个序列的信息来帮助我们进行预测
![截图: 一个图表展示了序列数据，其中输入序列 $x^{(1)}, x^{(2)}, x^{(3)}, \dots$ 依次连接，并且每个 $x^{(t)}$ 对应一个输出 $y^{(t)}$](10-414_深度学习系统课程笔记.assets/IMG-10-414_深度学习系统课程笔记-20250927205758073.png)
- 通常，我们将时间步 $t$ 的整个输入向量用下标表示为 $x_t$ 

### Example：词性标注 (Part of speech tagging)

- **任务**: 给定一个单词序列，确定每个单词的词性
- 一个单词的词性取决于它所在的上下文，而不仅仅是单词本身
<img src="10-414_深度学习系统课程笔记.assets/IMG-10-414_深度学习系统课程笔记-20250927205949797.png" alt="截图: 一个词性标注的例子，输入 &quot;The quick brown fox jumped&quot; 对应输出 &quot;DT JJ JJ NN VBD&quot;" style="zoom:33%;" />

### 示例：语音转文本 (Speech to text)

- **任务**: 给定一个音频信号（甚至假设我们知道单词的边界，并将每个片段映射到一个固定大小的向量描述符），确定相应的文本转录
- 同样，单词的上下文极其重要（例如，糟糕的语音识别系统可能会出错）
<img src="10-414_深度学习系统课程笔记.assets/IMG-10-414_深度学习系统课程笔记-20250927210218493.png" alt="截图: 一段音频波形图被分割并标注为 &quot;Deep Learning Systems Algorithms and Implementation&quot;" style="zoom:33%;" />

### 示例：自回归预测 (Autoregressive prediction)

- 这是一种特殊的序列预测，其要预测的元素是序列中的下一个元素
- 常见于时间序列预测、语言建模 (language modeling) 等用例
<img src="10-414_深度学习系统课程笔记.assets/IMG-10-414_深度学习系统课程笔记-20250927210244146.png" alt="截图: 一个自回归预测的例子，输入 &quot;The&quot; 预测 &quot;quick&quot;, 输入 &quot;quick&quot; 预测 &quot;brown&quot;, 依此类推" style="zoom:33%;" />

## 15.2 循环神经网络 (Recurrent Neural Networks, RNNs)

- 循环神经网络 (RNNs) 随时间维护一个隐藏状态 (hidden state)，该状态是当前输入和前一个隐藏状态的函数

<img src="10-414_深度学习系统课程笔记.assets/IMG-10-414_深度学习系统课程笔记-20250927210549967.png" alt="截图: RNN的基本结构图，展示了输入 $x_t$ 和前一时刻的隐藏状态 $h_{t-1}$ 如何共同计算出当前隐藏状态 $h_t$，并由 $h_t$ 产生输出 $y_t$" style="zoom:40%;" />

- **核心公式**:
  - 隐藏状态: $h_{t}=f(W_{hh}h_{t-1}+W_{hx}x_{t}+b_{h})$
  - 输出: $y_{t}=g(W_{yh}h_{t}+b_{y})$
- 其中 $f$ 和 $g$ 是激活函数，$W_{hh}$、$W_{hx}$、$W_{yh}$ 是权重(下标hx意为将x映射到h的维度)，$b_{h}$、$b_{y}$ 是偏置项

### 如何训练你的RNN

- 给定一个输入和目标输出序列 $(x_{1},...,x_{T},\ y_{1}^{*},...,y_{T}^{*})$，我们可以使用**时间反向传播 (backpropagation through time, BPTT)** 来训练RNN
- 这实际上就是将RNN按序列长度“展开”，然后主要依赖自动微分
...
### 堆叠RNNs (Stacking RNNs)

- 就像普通的神经网络一样，RNNs也可以堆叠在一起，将一层的隐藏单元作为下一层的输入，从而形成“深度RNNs (deep RNNs)”
- 实际上，“非常深”的RNNs的价值往往不如其他架构
<img src="10-414_深度学习系统课程笔记.assets/IMG-10-414_深度学习系统课程笔记-20250927214212267.png" alt="截图: 堆叠RNN的结构图，展示了第一层RNN的隐藏状态序列 h(1) 作为第二层RNN的输入序列" style="zoom:50%;" />

### 激活/梯度爆炸 (Exploding activations/gradients)

- 训练RNNs的挑战与训练深度MLP网络相似
- 因为我们在长序列上训练RNNs，如果RNN的权重/激活缩放不当，隐藏激活（以及梯度）会随着序列长度无限增长
![截图: 一张图表显示，随着时间步长(Time step t)的增加，隐藏单元的范数(Norm of hidden unit)呈指数级增长](10-414_深度学习系统课程笔记.assets/IMG-10-414_深度学习系统课程笔记-20250927214359563.png)

- **示例**: 单层RNN，使用ReLU激活函数，权重初始化为 $W_{hh}​∼N(0,3/n)$
- (回想一下，对于ReLU激活函数，σ2=2/n 是“合适的”初始化)
  
### 激活/梯度消失 (Vanishing activation/gradients)

- 同样，如果权重太小，来自输入的信息会随时间迅速衰减
- 而我们通常希望用序列模型来建模的正是“长距离依赖关系”

![截图: 一张图表显示，随着时间步长(Time step t)的增加，隐藏单元的范数(Norm of hidden unit)呈指数级衰减至零](10-414_深度学习系统课程笔记.assets/IMG-10-414_深度学习系统课程笔记-20250927215315642.png)

- **示例**: 单层RNN，使用ReLU激活函数，权重初始化为 $W_{hh}​∼N(0,1.5/n)$
- (这里仅在时间点1提供了非零输入，以展示关于该输入的信息随时间衰减的情况)

### 修改激活函数

- ReLU的一个明显问题是它可以无界增长；使用有界激活函数（如 `sigmoid` 或 `tanh`）能“修复”这个问题吗？
  
- **不能**: 
	- 可以解决激活值爆炸, 因为限制了输出范围
	- 要创建足够大的权重以避免激活/梯度消失，就需要让激活函数处于“饱和”区域，而在这些区域梯度非常小，因此**仍然存在梯度消失问题**
  

![截图: sigmoid函数和tanh函数的图像](10-414_深度学习系统课程笔记.assets/IMG-10-414_深度学习系统课程笔记-20250927222922383.png) 

---

## 15.3 LSTMs (Long Short-Term Memory)

### 长短期记忆RNNs

长短期记忆 (Long Short-Term Memory, LSTM) 单元是一种特殊形式的隐藏单元更新方式，它避免了（部分）普通RNNs的问题

- **步骤1**: 将隐藏单元分为两个部分，分别称为（有点令人困惑的）**隐藏状态 (hidden state)** $h_t$​ 和 **单元状态 (cell state)** $c_t$
  

![截图: 从RNN单元到LSTM单元的演变，图中显示LSTM单元内部包含 ht​ 和 ct​ 两个状态](10-414_深度学习系统课程笔记.assets/IMG-10-414_深度学习系统课程笔记-20250927230028887.png)

- **步骤2**: 使用一个非常特定的公式来更新隐藏状态和单元状态
  
- 为了更好地理解(maybe更困惑)，还引入了一些其他名称，如“遗忘门 (forget gate)”、“输入门 (input gate)”、“输出门 (output gate)”
$$
\begin{aligned}&\begin{bmatrix}i_t\\f_t\\g_t\\o_t\end{bmatrix}=\begin{pmatrix}\mathrm{sigmoid}\\\mathrm{sigmoid}\\\mathrm{tanh}\\\mathrm{sigmoid}\end{pmatrix}(W_{hh}h_{t-1}+\mathrm{W}_{hx}x_t+b_h)\\&c_{t}=c_{t-1}\circ f_{t}+i_{t}\circ g_{t}\\&\begin{aligned}h_t=\tanh(c_t)\circ o_t\end{aligned}\end{aligned}
$$
- $W_{hh}\in \mathbb R^{4d*d}, W_{hx}\in \mathbb R^{4d*n}$, 每个i, f, g, o都是d维的
### 为什么LSTMs有效？

- 关于“理解LSTMs工作原理”的论文/博客文章似乎数不胜数
- **关键在于这行公式**: $c_t​=c_{t−1}​∘f_t​+i_t​∘g_t$
  
- **核心思想**:
    - 我们通过将 $c_{t−1}$​ 按比例缩小（通过遗忘门 ft​，[0,1]组成的d维向量），然后再加上一项（通过输入门 it​ 和 gt​）来形成新的单元状态 ct​
    - 重要的是，ft​ 的 sigmoid 激活函数在值为1时达到“饱和”，此时会使 ct−1​ 不受影响地直接通过
    - 这意味着在更大范围的权重下，LSTMs**不会遭受梯度消失的困扰**
      

### 一些著名的LSTMs

- LSTM历史上一个非常著名的博客文章：[The Unreasonable Effectiveness of Recurrent Neural Networks by Andrej Karpathy](http://karpathy.github.io/2015/05/21/rnn-effectiveness/)
  

![截图: LSTM发明者之一Sepp Hochreiter的谷歌学术引用图表，图中显示在他的论文引用量在2015年Andrej Karpathy的博客文章发布后出现了爆炸性增长](10-414_深度学习系统课程笔记.assets/IMG-10-414_深度学习系统课程笔记-20250927234533529.png)

**示例：由模型生成的代码/文本**

- 由在**Linux源代码**上训练的字符级自回归模型生成:

```c
/*
* Increment the size file of the new incorrect
UI FILTER group information
...
- 在**LaTeX源代码**上训练生成的文本:
![](10-414_深度学习系统课程笔记.assets/IMG-10-414_深度学习系统课程笔记-20250927235250975.png)

## 15.4 超越“简单”序列模型

### 序列到序列 (Sequence-to-sequence) 模型

- 为了让大家初步了解RNNs/LSTMs在“简单序列预测”之外的应用，我们来看看语言翻译任务
  
- **结构**:
    - 可以将两个RNNs连接在一起
    - 一个 **编码器 (encoder)** 只负责处理输入序列以生成最终的隐藏状态（没有损失函数）
    - 然后另一个 **解码器 (decoder)** 接收这个初始隐藏状态，只负责生成一个序列
      

![截图: Sequence-to-sequence模型结构图，一个RNN(编码器)读取输入"Translating language is difficult \<STOP\>"，其最终隐藏状态被传递给另一个RNN(解码器)以生成输出"Traducir un idioma es dificil \<STOP\>"](10-414_深度学习系统课程笔记.assets/IMG-10-414_深度学习系统课程笔记-20250927235311692.png)

### 双向RNNs (Bidirectional RNNs)

- **问题**: 标准RNNs只能使用直到时间点 t 的序列信息来预测 yt​
    - 这在某些情况下是合适的（例如，自回归模型）
    - 但有时却不合适（例如，在语言翻译中，我们希望使用“整个”输入序列的上下文）
    
- **解决方案**: 双向RNNs (Bi-directional RNNs)
    - 将一个**前向RNN**和一个**后向RNN**堆叠在一起
    - 这样，整个序列的信息都能传播到每个时间点的隐藏状态中
- 但是依赖过于复杂?

![截图: 双向RNN结构图，展示了信息同时从左到右和从右到左两个方向传播](10-414_深度学习系统课程笔记.assets/IMG-10-414_深度学习系统课程笔记-20250928000548588.png)




# Lec17 Transformers and Attention

## 17.1 时间序列建模的两种方法

### 时间序列预测

让我们回顾一下前几节课的基本时间序列预测任务

![时间序列预测模型](10-414_深度学习系统课程笔记.assets/IMG-10-414_深度学习系统课程笔记-20250927205758073.png)

- $x_1$ 预测 $y_1$
- $x_1, x_2$ 预测 $y_2$
- $x_1, x_2, x_3$ 预测 $y_3$

更根本地说，时间序列预测任务是预测 $y_{1:T}=f_{\theta}(x_{1:T})$ 其中 $y_t$ 只能依赖于 $x_{1:t}$

有多种方法可以做到这一点，这些方法可能涉及也可能不涉及 RNNs 的隐状态表示

### RNN "隐状态" (latent state) 方法

我们已经看过了 RNN 的时间序列处理方法：维护一个 "隐状态" $h_t$ 它总结了到该时间点为止的所有信息

![RNN 结构](10-414_深度学习系统课程笔记.assets/IMG-10-414_深度学习系统课程笔记-20250927210549967.png)

- **优点**: 潜在的“无限”历史，紧凑的表示 (h3隐含了前面所有时间的信息)
- **缺点**: 历史和当前时间之间的计算路径很长 ⇒ 导致梯度消失/爆炸，难以学习
  

### "直接预测" (direct prediction) 方法

相比之下，也可以直接预测输出 $y_t$ $y_t=f_{\theta}(x_{1:t})$ (只需要一个能够对不同长度的输入进行预测的函数)

![直接预测模型](10-414_深度学习系统课程笔记.assets/IMG-10-414_深度学习系统课程笔记-20251116152459573.png)

- **优点**: 通常从过去到当前状态的计算路径更短
- **缺点**: 没有紧凑的状态表示(对于新input x4, 必须保留前面所有input, 而不能像RNN只保留h3)，实践中历史长度有限

### 用于直接预测的 CNNs

指定函数 $f_{\theta}$ 最直接的方法之一是（全）卷积网络 也称为时间卷积网络 (Temporal Convolutional Networks, TCNs)

![堆叠的因果卷积 (causal convolutions)](10-414_深度学习系统课程笔记.assets/IMG-10-414_深度学习系统课程笔记-20251116152802711.png)

主要的约束是卷积必须是**因果 (causal)** 的： $z_t^{(i+1)}$ 只能依赖于 $z_{t-k:t}^{(i)}$

许多成功的应用：例如用于语音生成的 WaveNet (van den Oord et al., 2016)

#### CNNs 用于密集预测的挑战

尽管 CNNs 很简单，但它们在时间序列预测方面有一个显著的缺点： 每个卷积的感受野通常相对较小 → 需要很深的网络才能真正包含过去的信息

潜在的解决方案：

- **增加 kernel size**: 也会增加网络的参数
- **池化层 (Pooling layers)**: 不太适合密集预测，因为我们希望预测所有的 $y_{1:T}$ 
- **空洞卷积 (Dilated convolutions)**: “跳过”一些过去的状态/输入


## 17.2 Self-attention and Transformers

### 深度学习中的 "Attention"

深度网络中的 "Attention" (注意力) 通常指**任何对单个状态进行加权然后组合**的机制

![RNN 顶层添加 Attention](10-414_深度学习系统课程笔记.assets/IMG-10-414_深度学习系统课程笔记-20251116153810904.png)

- $z_{t}=\theta^{T}h_{t}^{(k)}$ ($\theta$是一个可学习参数)
- $w=softmax(z)$
- $\overline{h}=\sum_{t=1}^{T}w_{t}h_{t}^{(k)}$


最初用于 RNNs，当希望以更通用的方式（而不仅仅是看最后一个状态）**组合**所有时间的隐状态($h_t$)时使用

### Self-attention 操作

Self-attention (自注意力) 是指一种特定形式的注意力机制

给定三个输入 $Q, K, V \in \mathbb{R}^{T\times d}$ ("Queries", "Keys", "Values", 这是深度学习中语义最不明确的名称之一)

$$
Q=[\begin{matrix}-&q_{1}^{T}&-\\ -&q_{2}^{T}&-\\ &\vdots&\\ -&q_{T}^{T}&-\end{matrix}], K=[\begin{matrix}-&k_{1}^{T}&-\\ -&k_{2}^{T}&-\\ &\vdots&\\ -&k_{T}^{T}&-\end{matrix}], V=[\begin{matrix}-&v_{1}^{T}&-\\ -&v_{2}^{T}&-\\ &\vdots&\\ -&v_{T}^{T}&-\end{matrix}]
$$

我们将 self-attention 操作定义 为: 
$$
SelfAttention(Q, K, V) = softmax(\frac{QK^{T}}{\sqrt d})V
$$

- 这是一个 $T \times T$ 的 "权重" 矩阵
- $softmax$ 按行 (rowwise) 应用

**特性**:

- 对于 $Q, K, V$ 矩阵的排列具有不变性（实际上是等变性, equivariant）
- 允许所有时间的 $q_t, k_t, v_t$ 之间产生影响
- 计算成本为 $O(T^2 d)$ (由于非线性应用于完整的 $T \times T$ 矩阵，无法轻易降低)


### 用于时间序列的 Transformers

Transformer 架构使用一系列 attention 机制（和前馈层）来 处理时间序列
$$Z^{(i+1)}=Transformer(Z^{(i)})$$ (详情见下一张幻灯片)

所有时间步（实践中，在给定的时间片内） 都是并行处理的，避免了像 RNNs 那样的 顺序处理需求

![Transformer 层次结构](10-414_深度学习系统课程笔记.assets/IMG-10-414_深度学习系统课程笔记-20251116164244175.png)



### Transformer模块

更详细地说，Transformer 模块具有以下形式：
![Transformer Block 结构图](10-414_深度学习系统课程笔记.assets/IMG-10-414_深度学习系统课程笔记-20251116164350864.png)

- **Input** -> $Q, K, V$
- $Q, K$ -> Multiply ($QK^T$)
- -> Softmax (除以 $d^{1/2}$)
- -> Multiply (乘以 $V$) -> **Self-attention** (输出 $\tilde{Z}$)
- (Self-attention 输出) + (Input 的残差连接) -> Normalize (LayerNorm) -> $\tilde{Z}'$
- $\tilde{Z}'$ -> Feed Forward (FFN)
- (FFN 输出) + ($\tilde{Z}'$ 的残差连接) -> Normalize (LayerNorm) -> **Output**
...
\begin{aligned}
\tilde{Z}_{1}
:=&\text{SelfAttention}(Z^{(i)}W_{Q}, Z^{(i)}W_{K}, Z^{(i)}W_{V})
\\=&softmax(\frac{Z^{(i)}W_{Q}W_{K}^{T}{Z^{(i)}}^{T}}{d^{1/2}})Z^{(i)}W_{V}
\end{aligned}
$$
2. **Add & Norm 1**: $\tilde{Z}_{2}:=\text{LayerNorm}(Z^{(i)}+\tilde{Z}_{1})$ 
3. **Feed Forward & Add & Norm 2**: $Z^{(i+1)}:=\text{LayerNorm}(\tilde{Z}_{2} + \text{ReLU}(\tilde{Z}_{2}W_{1})W_{2})$ 


这有点复杂，但实际上只是 self-attention， 然后是一个线性层 + ReLU (即 FFN)， 并加入残差连接 (residual connections) 和归一化 (normalization) (这些组件的具体位置经常被调整)

### Transformers 应用于时间序列

我们可以将 Transformer block 应用于时间序列的“直接”预测方法， 而不是使用卷积 block

**优点**:
- 单层内具有完整的感受野（即可以立即使用过去的数据）
- 跨时间混合不会增加参数数量（与卷积不同）

**缺点**:
- 所有输出都依赖于所有输入（不适用于例如自回归 (autoregressive) 任务）
- 没有数据的排序信息（记住 transformers 对序列的排列是等变的）


### Masked self-attention (掩码自注意力)

为了解决 "acausal" (非因果) 依赖的问题，我们可以**掩盖 (mask)** $softmax$ 运算符，为任何 "未来时间步" 分配零权重

$softmax(\frac{QK^{T}}{d^{1/2}}-M)V$

其中 $M$ 是一个上三角矩阵 (对角线为0, 上三角为 $\infty$) $M=[\begin{matrix} 0 & \infty & \infty \\ 0 & 0 & \infty \\ 0 & 0 & 0 \end{matrix}]$ 

> 请注意，尽管从技术上讲，这意味着我们可以“避免创建Attention矩阵中的那些元素”， 但在实践中，先形成它们然后将它们掩盖掉通常更快（周一会详细介绍）

### Positional encodings (位置编码)

为了解决“顺序不变性” (order invariance) 的问题， 我们可以向输入添加**位置编码 (positional encoding)**， 将每个输入与其在序列中的位置关联起来

$$
X_{\text{encoded}} = X + P = X + \begin{bmatrix} \sin(\omega_1 \cdot 1) & \cdots & \sin(\omega_n \cdot 1) \\ \sin(\omega_1 \cdot 2) & \cdots & \sin(\omega_n \cdot 2) \\ \vdots & \ddots & \vdots \\ \sin(\omega_1 \cdot T) & \cdots & \sin(\omega_n \cdot T) \end{bmatrix}
$$

- $X \in \mathbb{R}^{T \times d}$ 是输入
- $P \in \mathbb{R}^{T \times d}$ 是位置编码矩阵

其中 $w_i$ 通常根据对数尺度选择 (实际上，是将位置编码添加到 $X$ 的 $d$ 维投影上)

## 17.3 Transformers beyond time series (简要)

### 超越时间序列的 Transformers

最近的工作发现，transformer blocks 在时间序列之外也非常强大

- **Vision Transformers**: 将 transformer 应用于图像（表示为一组 patch embeddings），在大型数据集上效果优于 CNNs
- **Graph Transformers**: 在注意力矩阵中捕获图结构

在所有情况下，都存在一些挑战：

- 如何表示数据，使得 $O(T^2)$ 操作是可行的
- 如何形成位置嵌入 (positional embeddings)
- 如何形成掩码矩阵 (mask matrix)


# Lec19 训练大型模型 (Training Large Models)

**机器学习系统三要素**
<img src="10-414_深度学习系统课程笔记.assets/IMG-10-414_深度学习系统课程笔记-20251117011109461.png" style="zoom:25%;" />

- 更大的数据集 -> 需要更大的模型容量 -> 继而对计算设备提出了要求
- 机器学习的成功是所有三个要素的结合
- 许多近期的进展需要我们将这三者都推向极限

今天我们将学习两个主题：
1. 如何减少显存(内存)消耗，以便我们将更大的模型放入单个设备
2. 如何扩展(scale up)训练过程

## 19.1 引言与显存节省技术

### 回顾：GPU 显存(内存)层级结构

![GPU 内存层级, 从 Global memory 到 Shared memory 到 Registers](10-414_深度学习系统课程笔记.assets/IMG-10-414_深度学习系统课程笔记-20250831174149845.png)

- **GPU memory (Global memory)**:
    - RTX3080 10GB
    - RTX3090 24GB
    - A100 40/80 GB
    - H100 80GB; H200 141 GB
- **Shared memory**: 每个核心 64 KB
- **Registers**

### 显存(内存)消耗的来源

![一个简化的典型训练计算图 (权重被省略，并隐含在梯度步骤中)](10-414_深度学习系统课程笔记.assets/IMG-10-414_深度学习系统课程笔记-20251117011217494.png)
  

**显存(内存)消耗的来源**:
- **Model weights** (模型权重)
- **Optimizer states** (优化器状态) (例如 $w_1, w_2, u_1, u_2$ 等)
- **Intermediate activation values** (中间激活值)
  
### 推理场景的显存节约

![仅前向传播的计算图](10-414_深度学习系统课程笔记.assets/IMG-10-414_深度学习系统课程笔记-20251117011329725.png)

- 我们只需要 $O(1)$ 的内存/显存来计算一个 N 层深度网络的最终输出
- 通过循环使用两个 buffers, 一个存储前序输出, 一个存储当前计算输出, 之前的计算结果都可以丢掉(覆盖)

### 训练中的激活值显存成本

![训练计算图, 前向传播的激活值被保存用于后向传播](10-414_深度学习系统课程笔记.assets/IMG-10-414_深度学习系统课程笔记-20251117012129937.png)

- 因为需要保留中间值 (checkpoint) 以用于梯度计算步骤
- 训练一个 N 层神经网络需要 $O(N)$ 的内存/显存
- 我们将使用以下简化视图来合并梯度和前向计算
  ![](10-414_深度学习系统课程笔记.assets/IMG-10-414_深度学习系统课程笔记-20251117012622738.png)
  
### 自动微分中的 Checkpointing 技术

![3个步骤展示 Checkpointing 过程](10-414_深度学习系统课程笔记.assets/IMG-10-414_深度学习系统课程笔记-20251117012634906.png)

- **Step 0**: 只 checkpoint (保存) 选定的节点 (图中的彩色节点)
- **Step 1, 2**: 在需要时，在小的片段中重算缺失的中间节点 
	- *我嘞个以算代存*

对于一个 N 层的神经网络, 如果我们每 K 层进行一次 checkpoint (上图K=2)
- **显存成本** = $O(N/K)$ (Checkpoint 成本) + $O(K)$ (重计算中分段的内存成本)
- 当选择 $K=\sqrt{N}$ 时, **总显存成本** = $O(\sqrt{N})$
达到平方根级别的亚线性(Sublinear)的内存开销

## 19.2 并行与分布式训练

> **核心思想**: 利用多个 (GPU) 设备（可能分布在多个工作节点上）来训练一个模型
...
- 每个 worker 访问 mini-batch 的一部分 (例如 $B/K$)  
- 每个 worker 运行梯度计算，然后将所有梯度加起来 (sum up)
- 每个 worker 运行模型的**同一个副本** (replica)
  

#### Allreduce 通信接口抽象

- **接口**: `result = allreduce(float buffer[size])`
  
- **运行示例**:
![](10-414_深度学习系统课程笔记.assets/IMG-10-414_深度学习系统课程笔记-20251117014830837.png)

#### 通过 Allreduce 实现DP

![多个副本并行运行, 梯度 g1, g2 通过 Networks (allreduce) 汇总](10-414_深度学习系统课程笔记.assets/IMG-10-414_深度学习系统课程笔记-20251117015118965.png)

- 许多相同图的副本 (replicas) 在并行运行
- $G_1 \leftarrow sum(g_1 \text{ over replicas})$ 
- 梯度更新$w_1 \leftarrow w_1 - \alpha G_1$ 

#### Parameter Server (PS) 抽象

![key-value store 和 examples 交互](10-414_深度学习系统课程笔记.assets/IMG-10-414_深度学习系统课程笔记-20251117135133301.png)

- **接口**:
    - `ps.push(index, gradient)`
    - `ps.pull(index)`
    
- 在服务器 (key-value store) 上执行权重更新
  

#### 通过 Parameter Server 实现DP训练

![梯度 g1 被 push 到 Parameter server, 更新后的 w1 被 pull 下来](10-414_深度学习系统课程笔记.assets/IMG-10-414_深度学习系统课程笔记-20251117135313963.png)

- 许多相同图的副本在并行运行
- $w_1 \leftarrow w_1 - \alpha \text{ sum}(g_1 \text{ over replicas})$
- 在远程服务器上更新结果，并将更新后的结果发送回来

*Parameter Server和All Reduce的区别在于有一个中心服务器?* 
- 带来更复杂的调度方案的可行性: 
	- 比如不等待慢worker的参数, 提前更新
	- 或者worker崩溃, 可以从Parameter Server处拉取权重



### 通信与计算的overlap

![Epoch T 和 Epoch T+1 的计算图并行执行](10-414_深度学习系统课程笔记.assets/IMG-10-414_深度学习系统课程笔记-20251117135712297.png)

- **Epoch T**:
    - 计算 $g_1$ -> `Sync and update w1`
    - 计算 $g_2$ -> `Sync and update w2`
- **Epoch T+1**: ...
  
- 在通过网络发送数据的同时，有很多机会可以继续计算
	- 比如在上图中, g1处于计算图的前部, 当g1完成w1的更新后, 就可以先开始EpochT+1的前面部分计算, 不需要等待g2

### Model Parallel Training

![一个 RNN 模型被按层切分到 3 个 worker 上](10-414_深度学习系统课程笔记.assets/IMG-10-414_深度学习系统课程笔记-20251117014324873.png)

- 将计算图的各个部分映射到不同的 workers
![基于计算图的分割](10-414_深度学习系统课程笔记.assets/IMG-10-414_深度学习系统课程笔记-20251117014419058.png)

- 分割计算图, 在边界处放置 `send/recv` 对
- 更进一步分割, 可以实现流水线并行(Pipeline Parallelism)

### Tensor Parallelism
#### Tensor Parallelism (TP) - 模式1 (Row-Parallel)

![Y = W @ X , 其中 W 被按行切分](10-414_深度学习系统课程笔记.assets/IMG-10-414_深度学习系统课程笔记-20251117140433093.png)

- **线性层 (Linear layer)**: $Y = W @ X$
	- $W$ (权重) 被按行切分到 GPU0 和 GPU1
	- $X$ (输入) 是 replicated (复制的)
	- $Y$ (输出) 是 sharded (分片的, 按行)
  
- **问题**: 如何将分片的 $Y$ 馈送到下一层 (**如果下一层需要 replicated 输入**)？
  
#### Allgather 抽象

- **接口**: `result = allgather(float buffer[size])`
![](10-414_深度学习系统课程笔记.assets/IMG-10-414_深度学习系统课程笔记-20251117140614278.png)
#### TP 解决方案 1: Allgather

![X (replicated) -> W (row-sharded) -> Y (row-sharded) -> allgather -> Y (replicated)](10-414_深度学习系统课程笔记.assets/IMG-10-414_深度学习系统课程笔记-20251117140625016.png)
  
- `Allgather` 将 sharded (分片) 视图转换为 replicated (复制) 视图
  
- **问题**: 如何避免 Allgather？ (Allgather 通信量大)
	- All Gather每个设备需要发送并接收$\frac{D}{N}\times (N-1)\approx D$, 整个网络中的通信量为$N\times D$

#### TP 解决方案 2: 模式 2 (Column-Parallel)

![Y = X @ W, 其中 X 被按列切分, W 被按行切分](10-414_深度学习系统课程笔记.assets/IMG-10-414_深度学习系统课程笔记-20251118233525588.png)

1. 这是一个 $Y = X_1@W_1 + X_2@W_2$ 的情况
	- $X$ (输入) 是 sharded (按列)
	- $W$ (权重) 是 sharded (按行)
2. 并行计算
	- GPU0 计算 $Y_1 = X_1 @ W_1$ 
	- GPU1 计算 $Y_2 = X_2 @ W_2$ 
  3. 合并结果: $Y$ (输出) 是 replicated (复制的)
	- `partial results` = $Y_1, Y_2$ 
	- `allreduce` (sum) -> $Y = Y_1 + Y_2$ 

#### TP 解决方案 3: 组合模式 (Transformer FFN)

![](10-414_深度学习系统课程笔记.assets/IMG-10-414_深度学习系统课程笔记-20251118234908969.png)

- 这是一个典型的 tensor parallel 流水线，涉及两次 matmuls (例如 Transformer FFN)
  
- **MatMul 1 (Row-Parallel)**: $Y = W1 @ X$
    - $X$ (replicated) -> $W1$ (row-sharded) -> $Y$ (row-sharded)
    - (不需要通信)
    
- **MatMul 2 (Column-Parallel)**: $Z = W2 @ Y$ 
    - $Y$ (row-sharded) -> $W2$ (col-sharded) -> $Z$ (partial results, $Z_a, Z_b$)
    - $Z_a = W2_a @ Y_a$
    - $Z_b = W2_b @ Y_b$
    
- **Allreduce**:
    - $Z = \text{allreduce}(Z_a + Z_b)$
    - $Z$ (输出) 是 replicated
    
- 这种方式在两次 MatMul 之间隐藏了通信开销
  

### 并行化总结

- **Model parallel training** (模型并行训练) 按计算图的各个部分进行分区
- **Data parallel training** (数据并行训练) 按数据进行分区
- 在所有情况下，利用机会将计算与通信重叠 (overlap compute with communication)
...
# Lec 20 Generative Models

## 20.1 Generative Adversarial Training

### 20.1.1 从分类器到生成器
![](10-414_深度学习系统课程笔记.assets/IMG-10-414_深度学习系统课程笔记-20251122174215853.png)
- **分类器 (Classifier)**:
    - 输入: 随机向量 $X$ (例如图像)。
    - 输出: 预测标签 $\hat{y}$ (例如数字 6)。
    - 目标: 使预测类别 $h_{\theta}(x)$ 接近真实标签 $y$。
        
- **生成器 (Generator)**:
    
    - 输入: 随机向量 (Random vector) $Z$。
        
    - 输出: 生成样本 (Generated samples)。
        
    - 目标: 使生成的样本分布"接近"目标数据分布 (Target data distribution)。
        
    - 例子: 数字生成器，输入随机噪声，输出看起来像手写数字的图像。
        

### 20.1.2 定义分布的"距离" (Distance of Distributions)

- **挑战**: 与监督分类设置不同，生成任务的"目标"不那么明显。
    
- **需求**: 为了建立有效的训练机制，我们需要定义生成数据集和真实数据集之间的"距离"，并以此驱动训练。
    
- **直观目标**: 文本上来说，我们真正想要的是确保生成的样本"看起来是真实的 (looks real)"。
    

### 20.1.3 通过 Oracle 判别器学习生成器 (Learn Generator through an Oracle Discriminator)

假设我们要生成数据去愚弄一个完美的判别器。

- **设置**:
    
    - **随机向量**: $z\sim\mathcal{N}(0,1)$。
        
    - **生成器** $G$: $G(z)$ 生成假数据 (fake data)。
        
    - **Oracle 判别器 (Oracle Discriminator)** $D$:
        
        - 输入: 数据 $x$ (真实的或生成的)。
            
        - 输出: 输入来自真实数据集的概率 (Real or fake)。
            
        - 公式: $D(x)=1/(1+e^{-h(x)})$ (Sigmoid 输出)。
            
- **生成器目标**:
    
    - 假设我们有一个 Oracle 判别器，它可以区分真实数据和假数据。
        
    - 我们需要训练生成器去"愚弄"这个 Oracle 判别器。
        
    - 即最大化判别器出错的损失 (或者最小化判别器正确识别假数据的概率)。
        
    - **Objective**: $max_{G}\{-E_{z\sim Noise}log(1-D(G(z))\}$
        

### 20.1.4 学习判别器 (Learning the Discriminator)

实际上我们没有 Oracle 判别器，但我们可以利用真实数据和生成的假数据来学习它。

- **判别器** $D$: 尝试区分真实数据 $x$ 和假数据 $G(z)$。
    
- **判别器目标**:
    
    - 最小化分类误差（即最大化正确分类真实数据和假数据的概率）。
        
    - **Objective**: $min_{D}\{-E_{x\sim Data}log~D(x)-E_{z\sim Noise}log(1-D(G(z))\}$
        

### 20.1.5 生成对抗网络 (Generative Adversarial Network, GAN)

将生成器和判别器结合起来，这就变成了一个 $D$ 和 $G$ 之间的"极小极大 (minimax)"博弈。
...
- **实际中的生成器优化**:
    ...
    - 在实践中，优化 $G$ 时通常不直接使用上述 minimax 中的项（容易导致梯度消失）。
        
    - **常用目标**: $min_{G}\{-E_{z\sim Noise}log(D(G(z))\}$。
        
    - 含义: 最大化判别器预测生成图像为"真实"的概率。
        

### 20.1.6 实践中的生成对抗训练 (GAN Training in Practice)

这是一个迭代过程 (Iterative process):

1. **判别器更新 (Discriminator update)**:
    
    - 采样一小批 (minibatch) 生成数据 $G(z)$。
        
    - 采样一小批真实数据 $x$。
        
    - 更新 $D$ 以最小化: $-E_{x\sim Data}log~D(x)-E_{z\sim Noise}log(1-D(G(z))\}$。
        
2. **生成器更新 (Generator update)**:
    
    - 采样一小批 $G(z)$。
        
    - 更新 $G$ 以最小化: $-E_{z\sim Noise}log(D(G(z))\}$ (通常通过给模型输入标签 1 来实现)。
        

### 20.1.7 经典架构: DCGAN (Deep Convolutional GAN)

- **生成器**: 使用反卷积 (Conv2dTranspose) 将随机向量 $z$ 变换为图像。
    
- **判别器**: 使用卷积单元对图像进行二分类预测。
    

### 20.1.8 CycleGAN: 图像到图像翻译 (Image to Image Translation)

- **任务**: 学习两个非配对 (Unpaired) 数据集合之间的转换。
    
    - 例如: 莫奈画作 $\leftrightarrow$ 照片，斑马 $\leftrightarrow$ 马，夏天 $\leftrightarrow$ 冬天。
...
- **循环一致性 (Cycle Consistency)**:
    ...
    - 核心思想: 向前映射再映射回来，应该回到原始图像。
        
    - $x \rightarrow G(x) \rightarrow F(G(x)) \approx x$。
        
    - 损失函数包含 $L_1$ 距离: $||F(G(x)) - x||_1$。
        
- **整体架构**:
    
    - $X \xrightarrow{G} \hat{Y} \xrightarrow{D_Y} \text{Real/Fake}$
        
    - $Y \xrightarrow{F} \hat{X} \xrightarrow{D_X} \text{Real/Fake}$
        
    - $X \xrightarrow{G} \hat{Y} \xrightarrow{F} X'$ (Cycle Consistency Loss)
        

## 20.2 扩散模型 (Diffusion Models)

### 20.2.1 随机微分方程 (Stochastic Differential Equation, SDE)

描述了 $x_t$ 在空间中的随机运动。

- **SDE 公式**:
    
    $$dx_{t}=f(x_{t})dt+\sqrt{2D(x_{t},t)}dW_{t}$$
    - **漂移项 (Drift term)** $f(x_t)$: 拉向模态 (pulls towards modes)。
        
    - **扩散项 (Diffusion term)** $\sqrt{2D(x_t, t)}$: 注入噪声 (inject noise)。
        
    - $W_t$: 维纳过程 (Wiener process)，即高斯白噪声。
        
- **离散模拟 (Discrete simulation)** (步长 $\eta_t$):
    
    $$x_{t+1}\leftarrow x_{t}+\eta_{t}f(x_{t})+\mathcal{N}(0,2\eta_{t}D(x_{t},t))$$

### 20.2.2 前向扩散过程 (Forward Diffusion Process)

将图像 $x_0$ 逐渐转化为白噪声 $x_T$。

- **前向 SDE**:
    
    $$dx_{t}=-\frac{1}{2}\beta(t)x_{t}dt+\sqrt{\beta(t)}dW_{t}$$
    - $-\frac{1}{2}\beta(t)x_{t}$: 漂移向 0。
        
    - $\sqrt{\beta(t)}$: 添加噪声。
        
- **离散形式**:
    
    $$x_{t+1}\leftarrow(1-\frac{1}{2}\eta_{t}\beta(t))x_{t}+\mathcal{N}(0,\eta_{t}\beta(t))$$
- **结果**: $q(x_T)$ 接近标准正态分布。
    

### 20.2.3 逆过程/去噪过程 (Reverse/Denoising Process)

如果我们能模拟逆过程，就可以从噪声 $x_T$ 生成数据分布 $x_0$。

- **逆向 SDE**:
    
    $$dx_{t}=-[\frac{1}{2}\beta(t)x_{t}-\beta(t)\nabla_{x_{t}}log~q(x_{t})]dt+\sqrt{\beta(t)}dW_{t}$$
    - 关键项是 **Score Function (分数函数)**: $\nabla_{x_{t}}log~q(x_{t})$。
        

### 20.2.4 分数匹配 (Score Matching)

我们需要估计分数函数 $\nabla_{x_{t}}log~q(x_{t})$。

- **近似**: 使用神经网络 $s_{\theta}(x_{t},t)$ 来近似分数。
    
- **原始目标**:
    
    $$min_{\theta}E_{t\sim U(0,T),x_{t}\sim q(x_{t})}||s_{\theta}(x_{t},t)-\nabla_{x_{t}}log~q(x_{t})||^{2}$$
- **问题**: 我们没有 $\nabla_{x_{t}}log~q(x_{t})$ 的闭式解。
    

### 20.2.5 基于条件分布的分数匹配

利用前向过程的性质，我们可以推导出 $q(x_t|x_0)$ 是高斯分布。

- **推导**:
    
    $$q(x_{t}|x_{0})=\mathcal{N}(\gamma_{t}x_{0},\sigma_{t}^{2}I)$$
    
    其中 $\gamma_{t}=e^{-\frac{1}{2}\int_{0}^{t}\beta(s)ds}, \sigma_{t}^{2}=1-e^{-\int_{0}^{t}\beta(s)ds}$。
    
- **采样** $x_t$:
    
    $$x_{t}=\gamma_{t}x_{0}+\sigma_{t}\epsilon, \quad \epsilon\sim\mathcal{N}(0,I)$$
- **简化 Score**:
    
    $$\nabla_{x_{t}}log~q(x_{t}|x_{0})=-\nabla_{x_{t}}\frac{(x_{t}-\gamma_{t}x_{0})^{2}}{2\sigma_{t}^{2}}=-\frac{x_{t}-\gamma_{t}x_{0}}{\sigma_{t}^{2}}=-\frac{\epsilon}{\sigma_{t}}$$
- **参数化**: 令 $s_{\theta}(x_{t},t)=-\frac{\epsilon_{\theta}(x_{t},t)}{\sigma_{t}}$，即网络预测噪声 $\epsilon$。
    
- **最终训练目标 (Training Objective)**:
    
    $$min_{\theta}E_{t\sim U(0,T),x_{0}\sim q(x_{0}),\epsilon\sim\mathcal{N}(0,I)}\frac{1}{\sigma_{t}^{2}}||\epsilon_{\theta}(x_{t},t)-\epsilon||^{2}$$
...
- **训练扩散模型 (Training)**:
    ...
    2. 采样 $x_0$ 和噪声 $\epsilon$。
        
    3. 计算 $x_t = \gamma_t x_0 + \sigma_t \epsilon$。
        
    4. 更新网络 $\epsilon_{\theta}(x_t, t)$ 以最小化预测噪声和真实噪声的差异: $||\epsilon_{\theta}(x_{t},t)-\epsilon||^{2}$ (通常加权 $\lambda_t = \sigma_t^2$)。
...
- **生成过程 (Generation Process)**:
    ...
    - 公式: $dx_{t}=-[\frac{1}{2}\beta(t)x_{t}-\frac{\beta(t)\epsilon_{\theta}(x_{t},t)}{\sigma_{t}}]dt+\sqrt{\beta(t)}dW_{t}$
        
    - 直觉: 在预测噪声的相反方向迈出一小步。
        

### 20.2.7 潜在空间扩散模型 (Latent Space Diffusion Models)

为了提高效率，如 Stable Diffusion 等模型在潜在空间 (Latent Space) 进行扩散过程。

- **流程**:
    
    1. **Pixel Space** $\to$ **Latent Space**: 使用编码器 $\mathcal{E}$ 将图像压缩为潜变量 $z$。
        
    2. **Diffusion Process**: 在 $z$ 上进行加噪和去噪 (Denoising U-Net)。
        
    3. **Conditioning**: 注入文本、语义图等条件信息 (通过 Cross-Attention)。
        
    4. **Latent Space** $\to$ **Pixel Space**: 使用解码器 $\mathcal{D}$ 将去噪后的 $z$ 重建为图像。
        

### 20.2.8 GAN vs Diffusion Models 对比

- **GAN**:
    
    - 生成器 $G(z)$ 通过**单步 (Single step)** 生成输出。
        
- **Diffusion Models**:
    
    - 学习**迭代细化 (Iterative refinement)** 过程，而不是单步生成。
        

## 20.3 深度学习的模块化 (Deep Learning is Modular)

### 20.3.1 模块化本质

Deep Learning 本质上是模块化的，就像乐高积木一样。

- **例子**: 多层残差网络 (Multi-layer Residual Net)。
    
    - 由多个残差块 (Residual Block) 堆叠而成。
        
    - 残差块内部包含 Linear, ReLU, Matmul 等操作。
...
- **GAN Loss**:
    ...
    - GAN 不仅仅是一个模型，更像是一种损失函数机制。
        
    - 它涉及迭代更新配方。
        
    - 我们可以像使用 Loss Function 一样将其与其他神经网络模块组合。
        
- **用途**: 当我们希望一组生成数据"看起来像"另一组数据时，就可以使用 GAN "Loss"。
    
    - 例如 CycleGAN 就是将 GAN 模块用于无配对图像翻译任务中。
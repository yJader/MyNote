...
####  通过 Oracle 判别器 学习 生成器

假设我们要生成数据去愚弄一个完美的判别器(oracle, 预言机)。
![](10-414_深度学习系统课程笔记.assets/IMG-10-414_深度学习系统课程笔记-20251122232153708.png)
- **设置**:
...
- **判别器Discriminator** $D$: 尝试区分真实数据 $x$ 和假数据 $G(z)$。
- **判别器目标**:
...
1. **判别器更新 (Discriminator update)**:
    ...
    - 采样一小批真实数据 $x$。
    - 更新 $D$ 以最小化: $min_{D}\{-E_{x\sim Data}log~D(x)-E_{z\sim Noise}log(1-D(G(z))\}$。
2. **生成器更新 (Generator update)**:
    - 采样一小批 $G(z)$。
    - 更新 $G$ 以最小化: $-E_{z\sim Noise}log(D(G(z))\}$ (通常通过给模型输入标签 1 来实现)。

二者互相追赶, 可以达到一个相当强大的目标函数
但是minimax过程使得较难收敛

###  经典架构: DCGAN (Deep Convolutional GAN)

- **生成器**: 使用反卷积 (Conv2dTranspose) 将随机向量 $z$ 变换为图像。
- **判别器**: 使用卷积单元对图像进行二分类预测。
    

###  CycleGAN: 图像到图像翻译 (Image to Image Translation)
...
###  逆过程/去噪过程 (Reverse/Denoising Process)
...
###  潜在空间扩散模型 (Latent Space Diffusion Models)

...
- 笔记需要涵盖 PPT 的**所有**内容，包括概念、定理、例子、图表信息等, 如果需要插入截图, 请用`![图片描述]()`标注出来
- 每行末尾不需要加上`。`
- 结构清晰，适当使用标题（`#`）、代码块、内嵌latex公式等 Markdown 语法, 其中标题为# Lec[课程号] (参考文件名) [标题], ## [课程号].1, ## [课程号].2...
- 不需要严格按照ppt分页去划分大纲, 可以将相似内容总结合并或分出子级大纲
```
...
| 10/29      | 17 - Transformers and Autoregressive Models                     | Kolter     | [pdf](https://dlsyscourse.org/slides/transformers.pdf)                                                                             | [Youtube](https://youtu.be/IFKRf-BAqZo) / [Bilibili](https://www.bilibili.com/video/BV1ny411b7dJ/?p=22) / [B站](https://www.bilibili.com/video/BV1Rg4y137jH?p=22)                                                                        |                                                                                |
